...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($saldo_cc < 0) $saldo_cc = 0;

    $metodo_pago = 'varios';

    // NUEVO: insertar también el campo `total`
    ...
    $stmt->bind_param("iissidddddsdi",
        $cliente_id, $plan_id, $fecha_inicio, $fecha_vencimiento, $clases_disponibles,
        $precio, $otros_pagos, $descuento, $total_abonado, $metodo_pago, $saldo_cc, $total_pagar, $gimnasio_id
    );

    $stmt->execute();
    $membresia_id = $conexion->insert_id;

    // Guardar deuda si corresponde
    if ($saldo_cc > 0) {
