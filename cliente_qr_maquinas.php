<?php
// cliente_scan_qr.php — Panel del Cliente: escanear QR con la cámara y abrir la rutina (WebView-safe)
...
/* Consejo: si tu hosting permite, habilitá esta cabecera */
...
<body>
  <div class="wrap">
    ...
    <p class="muted">Apuntá la cámara al QR pegado en la máquina. Al detectar, te llevamos a la rutina donde podés elegir tu <strong>nivel</strong>.</p>
  ...
  <script>
    ...
    async function startHtml5(){
      ...
      try{
        ...
        await html5q.start({ facingMode:'environment' }, { fps:12, qrbox:(vw,vh)=>Math.min(300, Math.floor(Math.min(vw,vh)*0.7)) },
        ...
        showNotice('No se pudo usar la cámara en esta app. Tocá "Abrir en navegador" para continuar.');
      }
    }
    ...
    async function startCamera(){
      ...
      if (inApp){
        // Muchos wrappers bloquean getUserMedia → vamos directo al compatible
        status('Inicializando cámara (modo compatible)…');
        await stopCamera();
        return startHtml5();
      }

      // Navegador: HTTPS requerido (excepto localhost)
      if (!location.protocol.startsWith('https') && location.hostname!=='localhost'){
        status('Necesitás HTTPS para cámara en navegador. Intentando modo compatible…', true);
        return startHtml5();
      }
    ...
    async function toggleTorch(){
      ...
      try{ await track.applyConstraints({ advanced:[{ torch: !(torchOn=!torchOn) ? false : true }] }); status(torchOn?'Linterna encendida.':'Linterna apagada.'); }
    ...
    // Abrir esta misma página en el navegador del sistema (Chrome) — útil si la app bloquea la cámara
    btnExternal.addEventListener('click', ()=>{
      ...
      // Intent (Android) → si no funciona, fallback a tab nueva:
      const newTab = window.open(intent, '_self');
      if (!newTab) window.open(location.href, '_blank');
    });

    // Fallback: abrir por foto
    inputFile.addEventListener('change', ()=>{
      const file = inputFile.files && inputFile.files[0]; if (!file) return;
      status('Foto cargada. Si el sistema detecta la URL del QR, podrás abrirla.', false);
    ...
    nRetry.addEventListener('click', ()=>{ hideNotice(); stopCamera(); startCamera(); });
