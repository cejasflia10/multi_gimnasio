...
class QRvect {
    ...
    //----------------------------------------------------------------------  
    public static function eps($frame, $back_color = 0xFFFFFF, $fore_color = 0x000000, $filename = false, $pixelPerPoint = 4, $outerFrame = 4, $saveandprint = FALSE, $cmyk = false)
    {
    ...
    private static function vectSVG($frame, $pixelPerPoint = 4, $outerFrame = 4, $back_color = 0xFFFFFF, $fore_color = 0x000000)
    {
        ...
        if (!empty($back_color)) {
            ...
            $output .= '<rect width="' . ($imgW * $pixelPerPoint) . '" height="' . ($imgH * $pixelPerPoint) . '" fill="#' . $backgroundcolor . '" />' . "\n";
        }
