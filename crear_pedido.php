...
/* URL absoluta desde base */
function abs_url(string $rel): string {
  $base = defined('APP_BASE_URL') ? APP_BASE_URL : rtrim(getenv('APP_BASE_URL') ?: '', '/');
  if ($base) return rtrim($base,'/').'/'.ltrim($rel,'/');
...
try{
  ...
  /* ===== WhatsApp al organizador (si tiene número configurado) ===== */
  $wa_num = null; $evento_titulo='';
  $r = $conexion->prepare("SELECT e.titulo, cfg.whatsapp_organizador FROM eventos_deportivos e LEFT JOIN eventos_pagos_config cfg ON cfg.evento_id=e.id WHERE e.id=?");
  $r->bind_param('i',$evento_id); $r->execute(); $row=$r->get_result()->fetch_assoc(); $r->close();
  if ($row){ $evento_titulo = (string)$row['titulo']; $wa_num = preg_replace('/\D+/','', (string)($row['whatsapp_organizador'] ?? '')); }

  $base     = rtrim(abs_url(''),'/');
  $detail   = abs_url('ver_ventas_evento.php?evento_id='.$evento_id);
  $compURL  = $comprobante_rel ? abs_url($comprobante_rel) : null;

  $_SESSION['ok_msg'] = 'Pedido recibido. Te avisamos por email cuando se acredite el pago.';
  $_SESSION['wa_link'] = null;
  $_SESSION['wa_msg']  = null;

  if ($wa_num){
    $items = [];
    foreach($qty as $tid=>$q){
      if ($q<=0) continue;
      $items[] = sprintf('- %s x %d ($ %.2f c/u)', $tipos[$tid]['nombre'], $q, (float)$tipos[$tid]['precio']);
    }
    $wa_text  = [];
    $wa_text[] = '📲 *Nuevo pedido #'.$pedido_id.'*';
    $wa_text[] = '*Evento:* '.$evento_titulo;
    $wa_text[] = '*Cliente:* '.$nombre.' | '.$email.($tel? ' | +'.$tel : '');
    $wa_text[] = '*Pago:* '.$metodo_pago;
    $wa_text[] = '*Total:* $ '.number_format($total,2,',','.');
    if ($items){ $wa_text[] = "*Detalle:*"; $wa_text[] = implode("%0A", array_map('rawurlencode',[])); } // placeholder
    $wa_text[] = rawurlencode(implode("\n", $items));
    if ($compURL){ $wa_text[] = '*Comprobante:* '.$compURL; }
    $wa_text[] = '▶️ Revisar: '.$detail;

    $msg = implode("%0A", array_map(fn($s)=>rawurlencode($s), [
      ...
      '*Detalle:*',
      implode("\n", $items),
      ($compURL? ('*Comprobante:* '.$compURL) : ''),
      '▶️ Revisar: '.$detail,
    ]));

    $_SESSION['wa_link'] = 'https://wa.me/'.$wa_num.'?text='.$msg;
    $_SESSION['wa_msg']  = 'También podés avisar al organizador por WhatsApp.';
  }
