...
require __DIR__.'/factura_pdf.php'; // helper pdf (sin utf8_decode)
...
$vuelto             = (float)($_POST['vuelto'] ?? 0); // opcional

// Listas de items (opcional; si no vienen, el PDF crea un renglón “Productos”)
...
/** Devuelve un cliente_id. Si no hay, intenta encontrar por nombre; si no, crea temporal. */
...
/** Inserta DEBE en cc_movimientos */
...
// Texto medios
$metodos_txt = sprintf(
  "Efectivo: %.2f, Transf: %.2f, Débito: %.2f, Crédito: %.2f, Cuenta Corriente: %.2f%s",
  $pago_efectivo, $pago_transferencia, $pago_debito, $pago_credito, $pago_cuenta_corriente,
  ($vuelto > 0.009 ? ", Vuelto: ".number_format($vuelto, 2, ',', '.') : "")
);

// ====== Transacción ======
$conexion->begin_transaction();
try {
    // 1) Guardar venta resumida (tu tabla actual)
    $stmt = $conexion->prepare("
        INSERT INTO ventas_productos
          (cliente_nombre, cliente_temporal, descuento, total, fecha, hora,
           efectivo, transferencia, debito, credito, cuenta_corriente, gimnasio_id, metodos_txt)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ");
    ...
    $stmt->bind_param(
        ...
        $pago_efectivo, $pago_transferencia, $pago_debito, $pago_credito, $pago_cuenta_corriente, $gimnasio_id, $metodos_txt
    );
    if (!$stmt->execute()) { throw new Exception("Exec ventas_productos: ".$stmt->error); }
    $venta_id = (int)$stmt->insert_id;
    $stmt->close();

    // 2) Si hay CC, asegurar cliente_id y registrar DEBE
    if ($pago_cuenta_corriente > 0.009) {
        ...
        if ($cliente_id_ok > 0) {
            ...
            // actualizamos variable para el PDF
    ...
    exit("❌ Error al guardar la venta: ".htmlspecialchars($e->getMessage()));
}

// ====== PDF con factura_pdf.php ======

// Datos del gimnasio (logo, cuit, etc.)
...
// Ítems para PDF (si no vinieron, creamos uno genérico)
$items = [];
