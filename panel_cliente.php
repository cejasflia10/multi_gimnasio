...
<h2 style="text-align: center;">Reservar Turno</h2>
...
$puede_reservar = $membresia && $membresia['clases_disponibles'] > 0;
...
<?php if (!$puede_reservar): ?>
    <p style="color: red; text-align: center;">No tenés clases disponibles o tu plan está vencido.</p>
<?php endif; ?>
...
<?php while ($turno = $turnos->fetch_assoc()): ?>
    <form method="POST" action="reservar_turno.php">
        <div style="background:#222; color:gold; margin:10px 0; padding:15px; border-radius:10px;">
            ...
            Profesor: <?= $turno['profesor'] ?><br>
            Cupos: <?= $turno['cupos'] ?><br>
            <input type="hidden" name="turno_id" value="<?= $turno['id'] ?>">
            <button type="submit" class="btn-reservar" <?= !$puede_reservar ? 'disabled' : '' ?>>Reservar</button>
        </div>
    </form>
<?php endwhile; ?>
...
$reservas = $conexion->query("
    SELECT r.id, r.fecha_reserva, t.dia, t.hora_inicio, t.hora_fin, p.apellido AS profesor
    FROM reservas r
...
<?php if ($reservas->num_rows > 0): ?>
    <?php while ($reserva = $reservas->fetch_assoc()): ?>
        <div style="background:#111; color:gold; margin:10px 0; padding:15px; border:1px solid #444; border-radius:10px;">
            <?= $reserva['dia'] ?> - <?= substr($reserva['hora_inicio'], 0, 5) ?> a <?= substr($reserva['hora_fin'], 0, 5) ?><br>
            Profesor: <?= $reserva['profesor'] ?><br>
            Fecha de reserva: <?= $reserva['fecha_reserva'] ?>
            <form method="POST" action="cancelar_reserva.php" style="margin-top:10px;">
                <input type="hidden" name="reserva_id" value="<?= $reserva['id'] ?>">
                <button type="submit" style="background:red; color:white; border:none; padding:5px 10px; border-radius:5px;">Cancelar</button>
            </form>
        </div>
    <?php endwhile; ?>
<?php else: ?>
    <p style="color:gray; text-align:center;">No tenés reservas activas.</p>
<?php endif; ?>
<h2 style="text-align: center;">Reservar Turno</h2>
