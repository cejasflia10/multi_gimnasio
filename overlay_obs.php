<?php
// overlay_obs.php — Estilo ESPN/UFC (barra inferior) para OBS
...
<style>
  ...
  .plate{
    ...
    background:#0b1020cc;           /* vidrio oscuro */
  ...
  .center{
    ...
    gap:4px; min-width:min(20vw, 380px);
  }
  .time{
    font-weight:900; line-height:1;
    font-size: clamp(38px, 6.6vw, 110px);
    letter-spacing:1px;
    text-shadow: 0 6px 22px #000c;
  }
  .round{
    font-size: clamp(12px, 1.1vw, 16px); letter-spacing:.3px;
    padding: 4px 10px; border-radius:999px;
    border:1px solid #ffffff22; background:#0b1020cc;
  }
  ...
  .paused{ color:#ffd36b }        /* Pausado */
  .rest{ color:#7abbff }          /* Descanso */
  .live{ color:#7dffa3 }          /* En vivo */

  /* Oculta logos si no hay */
...
function paint(){
  ...
  // Tags (peso / modalidad / división si vienen)
  ...
  if (typeof tm.remaining === 'number'){
    ...
    remain = (tm.dur_round||180); // fallback
  }
...
setInterval(paint, 250);   // refresco por si falla polling
</script>
</body>
</html>
