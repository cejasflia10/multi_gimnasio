<?php
// overlay_obs.php — Overlay estilo ESPN para OBS (barra inferior)
// Lee JSON de combate_en_vivo.php?ajax=estado&pelea_id=...
...
<style>
  ...
  .plate{display:flex;align-items:center;justify-content:flex-start;gap:12px;height:min(9vh,90px);padding:10px 16px;border-radius:14px;border:1px solid #ffffff26;background:#0b1020cc;backdrop-filter:blur(6px);box-shadow:0 6px 26px #0006}
  ...
  .paused{color:#ffd36b}.rest{color:#7abbff}.live{color:#7dffa3}
...
function paint(){
  ...
  // cronómetro desde epoch_inicio (server-agnostic)
  let remain = null;
  if (typeof tm.remaining === 'number'){
    ...
    const elapsed = Math.max(0, Math.floor(Date.now()/1000) - (tm.epoch_inicio||0));
...
// ciclos
pull();
setInterval(pull, 1000);   // pide JSON cada 1s
setInterval(paint, 250);   // refresca vista 4 fps con el último estado
</script>
</body>
</html>
