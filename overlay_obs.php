...
function paint(){
  ...
  let remain=null;
  if(typeof tm.remaining==='number'){ remain=tm.remaining; }
...
async function pull(){
  const base = location.origin; // mismo host que el overlay (evita CORS)
  ...
  try{
    ...
    errCount = 0;
    // estado visual de “OK”
    if(UI.status && UI.status.textContent==='CONECTANDO…'){ UI.status.textContent='EN VIVO'; UI.status.className='status live'; }
    paint();
  }catch(e){
    errCount++;
...
pull();
setInterval(pull, 1000);   // pide estado cada 1s con cache-buster
setInterval(paint, 250);   // re-pinta por seguridad
</script>
</body></html>
