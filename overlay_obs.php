<?php
// overlay_obs.php — Barra inferior estilo ESPN/UFC para OBS (transparente)
// Lee estado desde combate_en_vivo.php?ajax=estado&pelea_id=...
...
<style>
  ...
  .bar{width: min(92vw, 1800px); margin-bottom: min(3vh, 36px);
       display:grid; grid-template-columns: 1fr auto 1fr; align-items:center; gap:10px;}

  .plate{display:flex; align-items:center; gap:12px; height:min(9vh, 90px);
         padding:10px 16px; border-radius:14px; border:1px solid #ffffff26;
         background:#0b1020cc; backdrop-filter: blur(6px); box-shadow: 0 6px 26px #0006;}
  ...
  .name{font-weight:900; font-size: clamp(18px, 2.2vw, 32px); letter-spacing:.5px;
        text-transform:uppercase; text-shadow: 0 2px 10px #000a;
  ...
  .status{position:absolute; left:min(2.2vw, 42px); top:min(2.2vh, 42px);
          font-weight:800; font-size: clamp(12px, 1vw, 16px);
          padding: 6px 12px; border-radius:999px; border:1px solid #ffffff22; background:#0b1020cc; backdrop-filter: blur(6px)}
...
function paint(){
  ...
  // Cálculo de tiempo (necesita epoch_inicio que publica combate_en_vivo -> api_combate_estado.php)
  let remain=null;
...
async function pull(){
  // 🔧 URL RELATIVA: asume que overlay_obs.php y combate_en_vivo.php están en la misma carpeta
  // Si están en /multi_gimnasio/, ambos deberían estar ahí.
  const url = `combate_en_vivo.php?ajax=estado&pelea_id=${peleaId}&_=${Date.now()}`;
  try{
    ...
    last = j.data;
    errCount = 0;
    if (UI.status && UI.status.textContent==='CONECTANDO…'){
...
// Arranque y ciclos
pull();
setInterval(pull, 1000);  // polling 1s
setInterval(paint, 250);  // repinta por seguridad
</script>
</body>
</html>
